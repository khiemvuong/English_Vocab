...
const THEME_MAP = {
  blue: {
    hoverBorder: 'hover:border-white/25 hover:ring-2 hover:ring-white/5',
    hoverShadow: 'hover:shadow-black/20',
    cardBg: 'bg-white/[0.03] backdrop-blur-xl border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.25)]',
    ...
    startButtonBg: 'bg-white/5 hover:bg-white/10 backdrop-blur-xs transition-colors duration-200',
    startButtonText: 'text-white',
    startButtonBorder: 'border-white/10',
    startPing: 'bg-white/30',
    startDot: 'bg-white',
    flareBg: 'bg-linear-to-tl from-white/3 via-white/0 to-transparent',
  },
  amber: {
    hoverBorder: 'hover:border-white/25 hover:ring-2 hover:ring-white/5',
    hoverShadow: 'hover:shadow-black/20',
    ...
    <Link href={isAvailable ? href : "#"} prefetch={false} className={className}>
      <div 
        className={`flex flex-col h-full border rounded-2xl p-5 md:p-6 transition-all duration-300 relative overflow-hidden
          ${!isAvailable 
            ? 'border-white/5 opacity-45 cursor-not-allowed bg-white/1 backdrop-blur-md' 
            : `border-white/5 cursor-pointer hover:-translate-y-1 ${theme.cardBg} ${theme.hoverBorder} ${theme.hoverShadow}`}`}
      >
